...
import { UserPlus, CreditCard, ClipboardList, BookOpen, HelpCircle, FileText } from "lucide-react";
export function QuickActions() {
  ...
  const actions = [{
    ...
    action: () => {
      navigate('/students');
      // Trigger the add student dialog after navigation
      setTimeout(() => {
    ...
    action: () => {
      navigate('/fees');
      // Trigger the add payment dialog after navigation
      setTimeout(() => {
  ...
  return <Card className="glass-card">
      <CardHeader>
        <CardTitle className="font-geist text-xl">Quick Actions</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {actions.map((action, index) => <motion.div key={action.title} initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.05
              ...
              <Button variant="ghost" onClick={action.action} className={`h-auto p-4 flex flex-col items-center gap-2 w-full bg-gradient-to-br ${action.color} hover:shadow-md transition-all font-geist`}>
                {action.icon}
