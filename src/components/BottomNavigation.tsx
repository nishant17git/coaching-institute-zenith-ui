      ...
      >
        <div className="flex justify-between items-center">
          {navItems.map((item) => {
            const isActive =
              item.path === "/more"
                ? ["/more", "/tests", "/reports", "/settings"].includes(
                    currentPath
                  )
            ...
            return (
              <Link
                key={item.name}
                to={item.path}
                className="relative flex flex-col items-center justify-center rounded-xl min-w-0 flex-1 h-14 py-0 px-[32px]"
              >
                {isActive && (
                  <motion.div
                    layoutId="navIndicator"
                    className="absolute inset-0 bg-[#FF00AA]/10 rounded-2xl border border-[#FF00AA]/20"
                    transition={{
                      type: "spring",
                      stiffness: 600,
                      damping: 35,
                ...
                >
                  <motion.div
                    ...
                    className={cn(
                      "transition-colors duration-400 ease-out mb-0",
                      isActive ? "text-[#FF00AA]" : "text-gray-500"
                    )}
                  >
