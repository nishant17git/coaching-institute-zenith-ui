...
import { Search, BarChart3, Cog, BookOpen, ChevronRight, User, UserPlus, CreditCard, FileText, HelpCircle, Biohazard, Loader2, X, Command } from "lucide-react";
...
export default function More() {
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      ...
      searchPromises.push(supabase.from('students').select('*').or(`full_name.ilike.%${query}%,father_name.ilike.%${query}%,mother_name.ilike.%${query}%,contact_number.ilike.%${query}%,address.ilike.%${query}%`).limit(6));
      searchPromises.push(supabase.from('tests').select('*').or(`test_name.ilike.%${query}%,subject.ilike.%${query}%,test_type.ilike.%${query}%`).limit(4));
      searchPromises.push(supabase.from('test_results').select(`
            *,
            students!inner(full_name, class),
            tests!inner(test_name, subject)
          `).limit(4));
      searchPromises.push(supabase.from('fee_transactions').select(`
            *,
            students!inner(full_name)
          `).or(`purpose.ilike.%${query}%,payment_mode.ilike.%${query}%,receipt_number.ilike.%${query}%`).limit(4));
  ...
  // Group
  const groupedResults = Array.isArray(searchResults) ? searchResults.reduce((acc, result, index) => {
  ...
  return <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="max-w-md mx-auto py-6 sm:max-w-2xl lg:max-w-4xl px-0 bg-white">
        ...
        ease: "easeOut"
          ...
          <div className="grid grid-cols-3 gap-3">
            {quickActions.filter(action => !action.featured).map((action, index) => <motion.div key={action.title} initial={{
            opacity: 0,
            scale: 0.8,
            y: 20
          }} animate={{
        ...
        ease: "easeOut"
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            ...
            icon: <HelpCircle className="h-4 w-4" />,
            path: "/settings/about"
          }].map((option, index) => <motion.div key={option.title} initial={{
            opacity: 0,
            x: -20
          }} animate={{
            opacity: 1,
      ...
      <AnimatePresence>
        {isSearchOpen && <>
          ...
          duration: 0.3,
          ease: "easeOut"
        }} className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50" onClick={() => setIsSearchOpen(false)} />
              ...
              <div className="w-full max-w-xl">
                ...
                ease: "easeOut"
                    ...
                    <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-full px-4 py-2 shadow-inner">
                      ...
                      <motion.input ref={searchInputRef} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search for students, tests, fees, attendance..." className="flex-1 bg-transparent focus:outline-none text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 ml-3 text-base" initial={{
                    width: 0
                  }} animate={{
                    width: "100%"
                  }} transition={{
                    ...
                    ease: "easeOut"
                      ...
                      {isSearching && <Loader2 className="h-5 w-5 animate-spin text-gray-600 dark:text-gray-400 ml-2" />}
                      <Button variant="ghost" size="sm" onClick={() => setIsSearchOpen(false)} className="h-8 w-8 p-0 hover:bg-gray-200 dark:hover:bg-gray-800 ml-2 rounded-full transition-colors duration-200">
                ...
                delay: 0.1,
                ease: "easeOut"
              }} className="max-h-[60vh] sm:max-h-96 overflow-y-auto overscroll-contain px-2" ref={resultsContainerRef}>
                    {isSearching ? <div className="py-12 text-center">
                        <motion.div animate={{
                    rotate: 360
                  }} transition={{
                    repeat: Infinity,
                    ...
                    ease: "linear"
                      ...
                      </div> : searchResults.length === 0 && searchQuery ? <div className="py-12 text-center">
                      ...
                      </div> : searchQuery && searchResults.length > 0 ? <div className="space-y-2">
                        {(Object.entries(groupedResults) as [string, any[]][]).map(([category, results]) => <div key={category}>
                            ...
                            <div className="divide-y divide-gray-200 dark:divide-gray-700">
                              {results.map(result => <motion.div key={result.id} initial={{
                        opacity: 0,
                        x: -10
                      }} animate={{
                        opacity: 1,
                        ...
                        duration: 0.3,
                        ease: "easeOut"
                      }} className={cn("flex items-center p-4 rounded-lg transition-colors duration-200", selectedIndex === result.globalIndex ? "bg-blue-50 dark:bg-blue-900/20" : "hover:bg-gray-50 dark:hover:bg-gray-800/50")} onClick={() => handleSearchResultSelect(result)} onMouseEnter={() => setSelectedIndex(result.globalIndex)}>
                                  <div className={cn("p-2 rounded-lg flex items-center justify-center flex-shrink-0", selectedIndex === result.globalIndex ? "bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-400" : "bg-gray-100 dark:bg-gray-800 text-gray-500")}>
                                    {result.icon}
                                  </div>
                                  <div className="ml-3 flex-1 min-w-0">
                                    ...
                                    {result.description && <p className="text-xs text-gray-400 dark:text-gray-500 truncate">
                                        {result.description}
                                      </p>}
                                  </div>
                                  <ChevronRight className={cn("h-5 w-5 flex-shrink-0 transition-colors", selectedIndex === result.globalIndex ? "text-blue-500" : "text-gray-300 dark:text-gray-500")} />
                                </motion.div>)}
                            </div>
                          </div>)}
                      </div> : !searchQuery ? <div className="py-12 text-center">
                        <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-3">
