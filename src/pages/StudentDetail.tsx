...
export default function StudentDetail() {
  ...
  const { students, classes, updateStudent, deleteStudent } = useData();
  ...
  const addFeeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['student-fees', id] });
  ...
  const attendanceData = [
    ...
    { name: "Leave", value: attendanceStats?.leave || 0 }
  ];
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-fade-in pb-10 px-2 md:px-0">
      <StudentDetailHeader 
        student={student} 
      ...
      <Card className="rounded-xl overflow-hidden shadow-sm border-white/20">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="px-2 md:px-4 lg:px-6">
          <TabsList className="grid grid-cols-4 w-full max-w-2xl mx-auto my-4 md:my-6 h-auto">
            <TabsTrigger value="profile" className="text-xs md:text-sm px-2 md:px-3 py-2">Profile</TabsTrigger>
            <TabsTrigger value="fees" className="text-xs md:text-sm px-2 md:px-3 py-2">Fees</TabsTrigger>
            <TabsTrigger value="attendance" className="text-xs md:text-sm px-2 md:px-3 py-2">Attendance</TabsTrigger>
            <TabsTrigger value="tests" className="flex gap-1 items-center text-xs md:text-sm px-2 md:px-3 py-2">
              <FileText className="h-3 w-3 md:h-3.5 md:w-3.5" /> Tests
            </TabsTrigger>
          </TabsList>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                  ...
                  <Card className="shadow-sm">
                    ...
                    <CardContent>
                      <div className="text-xl md:text-2xl font-semibold text-blue-600">
                        {studentTests.length > 0 
                          ? Math.round(studentTests.reduce((acc, test) => acc + (test.percentage || 0), 0) / studentTests.length)
                          : 0}%
                      </div>
                    </CardContent>
                  </Card>
                  <Card className="shadow-sm">
                    ...
                    <CardContent>
                      <div className="text-xl md:text-2xl font-semibold text-green-600">
                        {studentTests.length > 0 
                          ? Math.max(...studentTests.map(test => test.percentage || 0))
                          : 0}%
                      </div>
                    </CardContent>
                  </Card>
                  <Card className="shadow-sm">
                    ...
                    <CardContent>
                      <div className="text-xl md:text-2xl font-semibold text-amber-600">
                        {studentTests.length > 0 
                          ? Math.round(studentTests.filter(test => (test.percentage || 0) >= 40).length / studentTests.length * 100)
                          : 0}%
                      </div>
                    </CardContent>
                  </Card>
                ...
                <Card className="shadow-sm">
                  ...
                  <CardContent className="space-y-3">
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-3">
                            ...
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => handleExportTestPDF(test)}
                              className="h-8 w-8"
                            >
                              <Download className="h-4 w-4 text-muted-foreground" />
                            </Button>
                    ...
                    {studentTests.length > 5 && (
                      <div className="text-center pt-2">
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={() => navigate(`/tests/history/${id}`)}
                        >
                          View All Tests
                        </Button>
                      </div>
