...
export default function Fees() {
  ...
  const addPaymentMutation = useMutation({
    mutationFn: async (paymentData: any) => {
      ...
      } = await supabase.from('fee_transactions').insert(formattedData).select();
  ...
  const updatePaymentMutation = useMutation({
    mutationFn: async (paymentData: any) => {
      ...
      const {
        data: transactionData,
        error: transactionError
      } = await supabase.from('fee_transactions').update(formattedData).eq('id', selectedTransaction.id).select();
      if (transactionError) throw transactionError;
  ...
  const deletePaymentMutation = useMutation({
    mutationFn: async () => {
      ...
      const {
        error: deleteError
      } = await supabase.from('fee_transactions').delete().eq('id', transaction.id);
  ...
  const filteredTransactions = feeTransactions.filter(transaction => {
    ...
    const statusMatches = statusFilter === "all" || statusFilter === "paid" && transaction.payment_mode !== null || statusFilter === "pending" && transaction.payment_mode === null;
    ...
    if (periodFilter !== "all") {
      ...
      if (periodFilter === "thisMonth") {
        ...
        periodMatches = transactionDate.getMonth() === lastMonth && transactionDate.getFullYear() === lastMonthYear;
